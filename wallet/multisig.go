...
func (w *Wallet) MakeMultiSigScript(addrs []bronutil.Address, nRequired int) ([]byte, error) {
	pubKeys := make([]*bronutil.AddressPubKey, len(addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*bronutil.AddressScriptHash, error) {
	var p2shAddr *bronutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if waddrmgr.IsError(err, waddrmgr.ErrDuplicateAddress) {
				...
				p2shAddr, _ = bronutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*bronutil.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
