...
func NewRecoveryState(recoveryWindow uint32) *RecoveryState {
	...
	return &RecoveryState{
		recoveryWindow:   recoveryWindow,
		scopes:           scopes,
		watchedOutPoints: make(map[wire.OutPoint]bronutil.Address),
	}
}
...
func (rs *RecoveryState) WatchedOutPoints() map[wire.OutPoint]bronutil.Address {
...
type BranchRecoveryState struct {
	...
	addresses map[uint32]bronutil.Address
...
func NewBranchRecoveryState(recoveryWindow uint32) *BranchRecoveryState {
	return &BranchRecoveryState{
		recoveryWindow:  recoveryWindow,
		addresses:       make(map[uint32]bronutil.Address),
...
func (brs *BranchRecoveryState) AddAddr(index uint32, addr bronutil.Address) {
	brs.addresses[index] = addr
}

// GetAddr returns the address derived from a given child index.
func (brs *BranchRecoveryState) GetAddr(index uint32) bronutil.Address {
...
func (brs *BranchRecoveryState) Addrs() map[uint32]bronutil.Address {
